...
struct HouseholdFormView: View {
    ...
    private var anniversarySection: some View {
        VStack(alignment: .leading, spacing: 10) {
            SectionHeader(title: "Important Dates", action: { showingAddAnniversary = true }, actionLabel: "Add")

            if !anniversaries.isEmpty {
                AppCard {
                    VStack(spacing: 0) {
                        ForEach(anniversaries, id: \.id) { ann in
                            HStack {
                                Image(systemName: "calendar.badge.checkmark")
                                    .foregroundStyle(.accent).frame(width: 28)
                                Text(ann.label as String).font(.bodyRegular).foregroundStyle(.primaryText)
                                Spacer()
                                ...
                                Button {
                                    ...
                                    Image(systemName: "minus.circle.fill").foregroundStyle(Color.destructive)
                                }
                            }
            ...
            NavigationStack {
                VStack(spacing: 20) {
                    AppCard {
                        VStack(spacing: 12) {
                            FormRow("Label") {
                                TextField("e.g. Wedding, First day at job, Moved in...", text: $newAnnLabel)
                                    .textFieldStyle(AppTextFieldStyle())
                            }
                    ...
                    Text("Add any date that matters — anniversaries, milestones, memorials, moves.")
                        .font(.captionRegular)
