...
struct HouseholdDetailView: View {
    ...
    private var membersSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            SectionHeader(title: "Members", action: { showingAddPerson = true }, actionLabel: "Add")
            ...
            AppCard {
                VStack(spacing: 0) {
                    ForEach(household.anniversaries) { ann in
                        HStack {
                            Image(systemName: importantDateIcon(ann.label))
                                .foregroundStyle(.accent)
                                .frame(width: 28)
                            Text(ann.label.isEmpty ? "Important Date" : ann.label)
                                .font(.bodyRegular)
                                .foregroundStyle(.primaryText)
                            Spacer()
                        ...
                        .padding(.vertical, 12)
                        if ann.id != household.anniversaries.last?.id {
    ...
    private func importantDateIcon(_ label: String) -> String {
        let l = label.lowercased()
        if l.contains("wedding") || l.contains("anniversary") || l.contains("marriage") { return "heart.fill" }
        if l.contains("birth") || l.contains("baby") { return "figure.and.child.holdinghands" }
        if l.contains("job") || l.contains("work") || l.contains("career") { return "briefcase.fill" }
        if l.contains("moved") || l.contains("house") || l.contains("home") { return "house.fill" }
        if l.contains("passed") || l.contains("memorial") || l.contains("remembrance") { return "leaf.fill" }
        if l.contains("graduation") || l.contains("school") { return "graduationcap.fill" }
