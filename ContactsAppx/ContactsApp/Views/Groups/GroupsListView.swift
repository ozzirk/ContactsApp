...
struct GroupsListView: View {
    ...
    // Smart groups: one per relationship type that has at least one person
    private var smartGroups: [SmartGroup] {
        let used = Set(persons.map { $0.relationshipType })
        return RelationshipType.allCases
            .filter { used.contains($0) }
            .map { rel in
                SmartGroup(
                    relationshipType: rel,
                    people: persons.filter { $0.relationshipType == rel }.sorted { $0.lastName < $1.lastName }
                )
            }
    }
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                ScrollView {
                    VStack(spacing: 28) {
                        // Smart groups (auto-derived)
                        if !smartGroups.isEmpty {
                            VStack(alignment: .leading, spacing: 10) {
                                SectionHeader(title: "Smart Groups")
                                VStack(spacing: 10) {
                                    ForEach(smartGroups) { smart in
                                        NavigationLink(destination: SmartGroupDetailView(smartGroup: smart)) {
                                            SmartGroupRow(smartGroup: smart)
                                        }
                                        .buttonStyle(.plain)
                                    }
                                }
                        ...
                        if smartGroups.isEmpty && groups.isEmpty {
                            EmptyStateView(
                                icon: "person.2.fill",
                                title: "No groups yet",
                                subtitle: "Groups appear automatically as you add contacts. You can also create custom groups."
                            )
                        }
...
// MARK: - Smart Group model (computed, not stored)

struct SmartGroup: Identifiable {
    var id: String { relationshipType.rawValue }
    var relationshipType: RelationshipType
    var people: [Person]

    var color: Color {
        switch relationshipType {
        case .family: return Color(red: 0.75, green: 0.44, blue: 0.33)      // terracotta
        case .friend: return Color(red: 0.56, green: 0.68, blue: 0.57)      // sage
        case .neighbor: return Color(red: 0.48, green: 0.55, blue: 0.63)    // slate
        case .colleague: return Color(red: 0.22, green: 0.35, blue: 0.53)   // navy
        case .acquaintance: return Color(red: 0.63, green: 0.60, blue: 0.57) // stone
        case .teacher: return Color(red: 0.44, green: 0.57, blue: 0.38)     // moss
        case .other: return Color(red: 0.85, green: 0.66, blue: 0.66)       // blush
        }
    }
}

struct SmartGroupRow: View {
            ...
            HStack(spacing: 14) {
                ZStack {
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .fill(smartGroup.color.opacity(0.18))
                        .frame(width: 50, height: 50)
                    Image(systemName: smartGroup.relationshipType.icon)
                        .font(.system(size: 20))
                        .foregroundStyle(smartGroup.color)
                }
                VStack(alignment: .leading, spacing: 3) {
                    Text(smartGroup.relationshipType.rawValue + "s")
                        .font(.titleSmall)
                        .foregroundStyle(.primaryText)
                    Text("\(smartGroup.people.count) \(smartGroup.people.count == 1 ? "person" : "people")")
                        .font(.captionRegular)
...
// MARK: - Smart Group Detail

struct SmartGroupDetailView: View {
    let smartGroup: SmartGroup
    ...
    var body: some View {
            ...
            ScrollView {
                VStack(spacing: 24) {
                    ...
                    AppCard {
                        HStack(spacing: 16) {
                            ...
                            VStack(alignment: .leading, spacing: 4) {
                                ...
                                Text("\(smartGroup.people.count) people")
                                    .font(.captionRegular).foregroundStyle(.secondaryText)
                                Label("Auto-updated from contact type", systemImage: "wand.and.stars")
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        AppCard {
                            VStack(spacing: 0) {
                                ForEach(smartGroup.people) { person in
                                    NavigationLink(destination: PersonDetailView(person: person)) {
                                        HStack(spacing: 12) {
                                            ...
                                            VStack(alignment: .leading, spacing: 2) {
                                                ...
                                                Text(person.household?.name ?? "")
                                                    .font(.captionRegular).foregroundStyle(.secondaryText)
                                            }
                                            Spacer()
                                            Image(systemName: "chevron.right")
                                                .font(.captionMedium).foregroundStyle(.tertiaryText)
                                        }
                                        .padding(.horizontal, 16).padding(.vertical, 12)
                                    }
                                    .buttonStyle(.plain)
                                    if person.id != smartGroup.people.last?.id {
        ...
        .navigationTitle(smartGroup.relationshipType.rawValue + "s")
        .navigationBarTitleDisplayMode(.large)
    }
}
